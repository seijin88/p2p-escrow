...
class P2PEscrow(gl.Contract):
    """P2P escrow with contact info for off-chain communication (P1)."""
    ...
    def post_offer(
        ...
        offer = {
            ...
            'expires_at': expires_at if expires_at is not None else self._now() + OFFER_EXPIRY,
        }
    ...
    def lock_order(self, offer_id: u256) -> u256:
        ...
        def leader_fn() -> typing.Any:
            ...
            if market_micro <= 0:
                return {'market_micro': quoted_rate * PRICE_SCALE, 'deviation_pct': 0, 'within_limit': True}
            quoted_micro = quoted_rate * PRICE_SCALE
            deviation = abs(quoted_micro - market_micro) * 100 // market_micro
            return {'market_micro': market_micro, 'deviation_pct': deviation, 'within_limit': deviation <= RATE_DEV_PCT}
    ...
    def mark_paid(self, tx_id: str) -> None:
        ...
        _require(self._now() <= trade['created_at'] + PAYMENT_WINDOW, 'Payment window expired')
        trade['payment_tx_id'] = tx_id
        trade['status'] = 'paid'
        trade['paid_at'] = self._now()
        self._save_trade(trade['trade_id'], trade)

    def _get_active_trade_for_buyer(self, addr: typing.Any) -> typing.Optional[dict]:
        try:
            trades_data = json.loads(self.trades.get(self._last_trade_id(str(addr)), '{}'))
    ...
    def _last_trade_id(self, addr: str) -> str:
        return addr
    ...
    def release_crypto(self) -> None:
        ...
        _require(self._now() >= trade['paid_at'] + RELEASE_WINDOW, 'Release window not open')
        trade['status'] = 'settled'
        trade['settled_at'] = self._now()
        self._save_trade(trade['trade_id'], trade)

    def _get_active_trade_for_seller(self, addr: typing.Any) -> typing.Optional[dict]:
        try:
            trades_data = json.loads(self.trades.get(self._last_trade_id(str(addr)), '{}'))
