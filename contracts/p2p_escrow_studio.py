...
class P2PEscrow(gl.Contract):
    ...
    def register_profile(self, bank: str, number: str, name: str, contact: str) -> None:
        self.profiles[self._addr_to_key(str(gl.message.sender_address))] = json.dumps({'bank': bank, 'number': number, 'name': name, 'contact': contact})
    ...
    def create_offer(self, token: str, fiat_currency: str, fiat_amount: str, rate: str, payment_methods: str) -> u256:
        offer_id = self.offer_counter
        self.offer_counter += u256(1)
        self.contract_balance += gl.message.value
        offer = json.dumps({'offer_id': int(offer_id), 'seller': str(gl.message.sender_address), 'token': token, 'crypto_amount': str(gl.message.value), 'fiat_currency': fiat_currency, 'fiat_amount': fiat_amount, 'rate': rate, 'payment_methods': payment_methods, 'status': 'open'})
    ...
    def lock_order(self, offer_id: u256) -> u256:
        ...
        trade = json.dumps({'trade_id': int(trade_id), 'offer_id': int(offer_id), 'seller': offer['seller'], 'buyer': str(gl.message.sender_address), 'token': offer['token'], 'crypto_amount': offer['crypto_amount'], 'fiat_currency': offer['fiat_currency'], 'fiat_amount': offer['fiat_amount'], 'proof_url': '', 'status': 'locked'})
